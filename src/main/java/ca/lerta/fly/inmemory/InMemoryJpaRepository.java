...
public class InMemoryJpaRepository<T, ID extends Serializable & Comparable<ID>> implements JpaRepository<T, ID> {
    ...
    private PrimaryKeyGenerator<ID> primaryKeyGenerator;

    public PrimaryKeyGenerator<ID> getPrimaryKeyGenerator() {
    ...
    public List<T> findAllById(Iterable<ID> ids) {
        ...
        for (ID id : ids) {
            T selectedEntity = findById(id).get();
    ...
    public <S extends T, R> R findBy(Example<S> example, Function<FetchableFluentQuery<S>, R> queryFunction) {
        throw new UnsupportedOperationException("findBy");
    }
